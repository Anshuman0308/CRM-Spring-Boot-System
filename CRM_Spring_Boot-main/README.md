...
1. **User opens browser** → Frontend loads (`index.html`)
2. **User fills form** → JavaScript captures data
3. **JavaScript makes API call** → Controller receives request
4. **Controller calls Service** → Business logic executed
5. **Service calls Repository** → Database operation performed
6. **Data flows back** → Response sent to frontend
7. **Frontend updates** → User sees result
